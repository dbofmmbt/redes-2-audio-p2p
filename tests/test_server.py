...
def test_register():
    check(
        payload={"action": "register", "port": 12345, "songs": ["song1"]},
        expected={"message": "user registered succesfully"},
    )


def test_register_again_fails():
    register_message = {"action": "register", "port": 123, "songs": ["my_song"]}
...
def test_unregister_removes_peer():
    ...
    send_request(
        client1, {"action": "register", "port": 123, "songs": ["song_from_client1"]}
    )
...
def test_list():
    ...
    _ = send_request(
        conn1, {"action": "register", "port": 123, "songs": ["song_from_conn1"]}
    )
